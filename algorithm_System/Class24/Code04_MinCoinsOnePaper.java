...
public class Code04_MinCoinsOnePaper {
    ...
    //dp2的时间复杂度为:O(arr长度)+O(货币种数*aim*每种货币的平均张数)
    ...
    public static int dp3(int[] arr, int aim) {
        ...
        //虽然是嵌套了很多循环，但是时间复杂度为O(货币中数*aim)
        //因为用到窗口内最小值的更新结构
        for (int i = N - 1; i >= 0; i--) {
            for (int mod = 0; mod < Math.min(aim + 1, c[i]); mod++) {
                //当前面值X
                //mod mod+x mod+2x  mod+3x
    ...
    public static int[] generateRandomArray(int N, int maxValue) {
        int[] arr = new int[(int) (Math.random() * (N + 1))];
    ...
    public static void main(String[] args) {
        ...
        System.out.println("=========");
        System.out.println("货币大量重复出现的情况下,");
        System.out.println("大数据测试dp3开始");
        maxLen = 20000000;
        aim = 10000;
        maxValue = 1000;
        arr = generateRandomArray(maxLen, maxValue);
        ...
        System.out.println("dp3运行时间:" + ans3 + ",dp3运行时间:" + (end - start) + "ms");
        System.out.println("大数据量测试dp3结束");
        System.out.println("=======");

        System.out.println("当货币量很少出现重复，dp2比dp3有常数时间优势");
        System.out.println("当货币量大量出现重复，dp3时间复杂度略优于dp2");
